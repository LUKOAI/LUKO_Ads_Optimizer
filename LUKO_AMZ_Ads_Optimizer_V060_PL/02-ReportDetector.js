...
//
// FIX V0.61:
// - identifying terms teraz używają REGEX (tak jak patterns)
// - Dodany BONUS za nazwę arkusza zawierającą kluczowe słowa
// - Obniżony próg minimum threshold z 0.5 do 0.35
// - Dodany alias ReportDetector = UltraReportDetector
...
class UltraReportDetector {
  constructor(logger) {
    ...
    this.reportTypes = {
      ...
      // FIX: Rozszerzone wzorce dla Search Term Report
      SEARCH_TERM_REPORT: {
        required: ['search', 'impressions'],  // FIX: zmienione z 'search_term' na 'search'
        identifying: ['customer.*search.*term', 'search.*term', 'targeting'],
        patterns: ['search.*term', 'suchbegriff', 'sponsored.*products.*search'],
      ...
      KEYWORD_REPORT: {
        ...
        identifying: ['keyword.*text', 'schluesselwort'],
        patterns: ['keyword', 'schluessel'],
  ...
  detectReports() {
    this.logger.log('STARTING REPORT DETECTION V6.1', 'INFO');
    ...
    sheets.forEach(sheet => {
      ...
      // Pomiń arkusze LUKO (ale NIE pomijaj arkuszy z raportami użytkownika)
      ...
      if (sheet.getLastRow() === 0 || sheet.getLastColumn() === 0) {
        this.logger.log(`Skipping empty sheet: ${sheetName}`, 'INFO');
        return;
      }
  ...
  hasTypicalAmazonHeaders(values) {
    ...
    const amazonPatterns = [
      ...
      'customer search term', 'search term', 'targeting', 'match type',
      // BULK file nagłówki
  ...
  detectReportType(headers, sheetName) {
    ...
    sortedTypes.forEach(([typeName, typeConfig]) => {
      const score = this.calculateTypeScore(headerText, nameText, normalizedHeaders, typeConfig, typeName);
    ...
    // FIX: Obniżony próg z 0.5 do 0.35
  ...
  // OBLICZANIE SCORE DLA TYPU RAPORTU - FIX: identifying używa teraz REGEX
  calculateTypeScore(headerText, nameText, normalizedHeaders, typeConfig, typeName) {
    let score = {
      required: 0,
      identifying: 0,
      patterns: 0,
    ...
    if (typeConfig.required) {
      ...
      // Jeśli nie znaleziono żadnego required, sprawdź jeszcze raz z regex
      if (score.required === 0) {
        const requiredFoundRegex = typeConfig.required.filter(req => {
          const regex = new RegExp(req, 'i');
          return normalizedHeaders.some(h => regex.test(h)) || regex.test(headerText);
        });
        score.required = (requiredFoundRegex.length / typeConfig.required.length) * 0.4;
      }
    ...
    if (typeConfig.excludes && typeConfig.excludes.length > 0) {
    ...
    // FIX: BONUS ZA NAZWĘ ARKUSZA (nowe - do 15% bonus)
    score.nameBonus = this.calculateNameBonus(nameText, typeName);

    // PRIORYTET BONUS (10% wagi)
    const priorityBonus = (typeConfig.priority / 100) * 0.1;

    score.total = score.required + score.identifying + score.patterns + score.excludes + score.nameBonus + priorityBonus;

    return score;
  }

  // FIX: NOWA FUNKCJA - Bonus za nazwę arkusza
  calculateNameBonus(sheetNameLower, typeName) {
    let bonus = 0;

    // Mapowanie typów na słowa kluczowe w nazwie arkusza
    const nameKeywords = {
      'SEARCH_TERM_REPORT': ['search', 'term', 'sponsored_products_search', 'search_term'],
      'BULK_REPORT': ['bulk', 'sp_bulk', 'sponsored_products'],
      'CAMPAIGN_REPORT': ['campaign', 'campaigns'],
      'KEYWORD_REPORT': ['keyword', 'keywords'],
      'TARGET_REPORT': ['target', 'targeting']
    };

    const keywords = nameKeywords[typeName] || [];

    keywords.forEach(keyword => {
      if (sheetNameLower.includes(keyword)) {
        bonus += 0.15; // 15% bonus za każde trafienie
      }
    });
...
// ====================================
// 01-Main.js używa "new ReportDetector()" ale klasa nazywa się UltraReportDetector
