// ===== 05-MetricsCalculator-FIXED.js - WERSJA 6.1 =====
// ====================================
// LUKO AMZ Ads Optimizer
// Version: 6.1 FIXED
// Author: Łukasz Koronczok, NetAnaliza
// ====================================
// FIX: Bezpieczne ładowanie getAcosSettings z fallbackiem
...
class MetricsCalculator {
  ...
  loadAcosSettings() {
    try {
      // FIX: Sprawdź czy funkcja istnieje
      ...
      this.logger.log(`Using default ACOS settings: ${error.message}`, 'WARNING');
    }
  ...
  calculateMetrics(integratedData, sourceSheet = null) {
    ...
    if (sourceSheet) {
      ...
      // V6.2: Używaj helper functions dla kompatybilności wstecznej
      ...
      const bulkSheet = typeof getBulkSourceSheet === 'function'
        ? getBulkSourceSheet()
        : ss.getSheetByName(LUKO_CONFIG.SHEETS.BULK_SOURCE) || ss.getSheetByName(LUKO_CONFIG.SHEETS.BULK_SOURCE_OLD);
  ...
  findColumnIndices(headers) {
    return {
      campaignName: this.findColumnIndex(headers, ['Campaign Name', 'Kampagnenname', 'Nazwa kampanii', 'Campaign']),
      adGroupName: this.findColumnIndex(headers, ['Ad Group Name', 'Ad Group', 'Nazwa grupy']),
      keyword: this.findColumnIndex(headers, ['Keyword Text', 'Customer Search Term', 'Targeting', 'Keyword']),
      ...
      spend: this.findColumnIndex(headers, ['Spend', 'Wydatki', 'Ausgaben', 'Cost']),
  ...
  analyzeTargetsExtended(data, indices, headers) {
    ...
    const basicMetrics = this.calculateBasicMetrics(data, indices);
    const avgOrderValue = basicMetrics.sales / Math.max(1, basicMetrics.orders);
    const avgCPC = basicMetrics.spend / Math.max(1, basicMetrics.clicks);

    // Próg kliknięć = AOV / (CPC * 3) - czyli ile kliknięć powinno dać przynajmniej 1 zamówienie
    const clickThresholdHigh = Math.max(10, Math.min(30, avgOrderValue / Math.max(0.01, avgCPC * 3)));
  ...
  analyzeCampaigns(data, headers) {
    const map = new Map();
    const cIdx = this.findColumnIndex(headers, ['Campaign Name', 'Kampagnenname', 'Nazwa kampanii', 'Campaign']);
