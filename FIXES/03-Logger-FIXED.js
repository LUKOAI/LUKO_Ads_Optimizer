// ===== 03-LOGGER-FIXED.JS - KOMPLETNY LOGGER =====
// ====================================
...
class LukoLogger {
  ...
  saveToSheet() {
    try {
      ...
      let debugSheetName = 'LUKO_Debug_Log';
      try {
        if (typeof LUKO_CONFIG !== 'undefined' && LUKO_CONFIG.SHEETS && LUKO_CONFIG.SHEETS.DEBUG) {
          debugSheetName = LUKO_CONFIG.SHEETS.DEBUG;
        }
      } catch (e) {
  ...
  // FIX: Dodane clear() dla kompatybilności
  clear() {
    this.logs = [];
    this.startTime = new Date();
  }
