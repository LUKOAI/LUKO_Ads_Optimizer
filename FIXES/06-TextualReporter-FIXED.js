// ===== 06-TextualReporter-FIXED.js - WERSJA 6.1 =====
// ====================================
// LUKO AMZ Ads Optimizer
// Version: 6.1 FIXED
// Author: Łukasz Koronczok, NetAnaliza
// ====================================
// FIX: Bezpieczny dostęp do LUKO_CONFIG z fallbackiem
...
class TextualReporter {
  ...
  generateReport(metrics, integratedData) {
    ...
    // FIX: Bezpieczny dostęp do nazwy arkusza
    let sheetName = 'LUKO_Textual_Report';
    try {
      if (typeof LUKO_CONFIG !== 'undefined' && LUKO_CONFIG.SHEETS && LUKO_CONFIG.SHEETS.TEXTUAL_REPORT) {
        sheetName = LUKO_CONFIG.SHEETS.TEXTUAL_REPORT;
      }
    } catch (e) {
  ...
  buildPolishReport(metrics, integratedData) {
    ...
    lines.push('📊 RAPORT WYDAJNOŚCI REKLAM AMAZON - LUKO ANALYZER V6.1');
    ...
    lines.push('═'.repeat(70));
    lines.push('📞 LUKO ANALYZER V6.1 - Kompleksowa analiza Amazon Ads');
