// ===== 02-REPORTDETECTOR.GS - KOMPLETNY DETEKTOR RAPORTOW =====
// ====================================
// LUKO AMZ Ads Optimizer
// Version: 0.60 FIXED
// Author: Lukasz Koronczok, NetAnaliza
// ====================================
//
// FIX: Dodano alias ReportDetector = UltraReportDetector na koncu pliku
// To naprawia blad "ReportDetector is not defined"
...
class UltraReportDetector {
  ...
  // SPRAWDZENIE CZY ZAWIERA TYPOWE NAGLOWKI AMAZON
  ...
  normalizeHeader(header) {
    ...
    return header
      ...
      .replace(/[()]/g, '')
      .replace(/\s+/g, '_')
      .replace(/[aouA]/g, match => {
        const map = { 'a': 'ae', 'o': 'oe', 'u': 'ue' };
  ...
  detectLanguage(headers) {
    ...
    const languageMarkers = {
      'DE': ['ausgaben', 'verkaufe', 'klicks', 'impressionen', 'kampagnen', 'bestellungen'],
      'EN': ['spend', 'sales', 'clicks', 'impressions', 'campaigns', 'orders'],
      'PL': ['wydatki', 'sprzedaz', 'klikniecia', 'wyswietlenia', 'kampanie', 'zamowienia'],
      'FR': ['depenses', 'ventes', 'clics', 'impressions', 'campagnes', 'commandes']
    };
    ...
    try {
      ...
      headers.forEach((header, index) => {
        ...
        if (headerLower.includes('impression') || headerLower.includes('click') ||
            ...
            headerLower.includes('ausgaben') || headerLower.includes('verkaufe')) {
          numericColumns++;
        }
      });
...
// ====================================
// KRYTYCZNY FIX: Alias dla kompatybilnosci
// ====================================
// 01-Main-FIXED.js uzywa "new ReportDetector()" ale klasa nazywa sie UltraReportDetector
// Ten alias naprawia blad "ReportDetector is not defined"
